...
- 生成或修改任何 Ground、Resource、Decoration、Structure、Unit、Transition 或其他美术资源前，必须先阅读项目内的 [docs/asset-generation-spec.md](docs/asset-generation-spec.md)，并将其作为当前项目的默认生成规范。
- 从资源规范 v4.0 起，新生成及玩家指定的美术直接保留原始高清 PNG，不再强制转换为像素风或占地×32 的正式文件尺寸。
- 32×32 Tile 网格只定义逻辑占地、碰撞、寻路与布局；纹理在 Godot 中通过等比缩放适配世界、卡牌、HUD 和气泡显示，不得拉伸变形。
- 生成提示词必须明确用途、主体、透明度和禁止文字水印；只有具体玩法需要时才限定视角与光源，不再统一要求硬像素边缘。
- 生成后必须检查 RGBA/透明度、主体完整性、原始尺寸质量、缩放后的游戏内可读性和 Godot 导入设置。
